...
LED_G_tracking = GPIO.PWM(LED_G, 500)

########
GPIO.setup(LED_R, GPIO.OUT)
LED_R_downward = GPIO.PWM(LED_R, 500)
#############
...
LED_G_tracking.start(100)

LED_G_tracking.ChangeDutyCycle(100)
...
########################## database #############################
def conditions_fromDB(db,cur):
    db = pymysql.connect(host = '219.241.123.226',port = 10122, user = 'inventist', password = 'bems', db = 'mysql', charset = 'utf8')

    cur = db.cursor()
    sql = 'SELECT panel_flip FROM BEMS ORDER BY DATATIME DESC LIMIT 1'
    
    cur.execute(sql)
    
    result = cur.fetchall()

    condition = result[0][0].decode('utf8')
    
    print('condition : ',condition)
    print("")
    
    condition_result = int(str(result[0][0])[2])
...
def panel_downward():
    ...
    while True:
        condition = conditions_fromDB(db,cur)
        if condition != 0:
            print('Damaged or Polluted')
            servo_control_verti(0,0.2) #have to fix with complete downward angle...!
            servo_verti.stop()
            servo_horiz.stop()
            
            LED_G_tracking.stop()
            
            LED_R_downward.start(100)
            LED_R_downward.ChangeDutyCycle(100)
            
            time.sleep(7)
            
            print('Panel downward complete')
            LED_R_downward.stop()
            GPIO.cleanup()
            break;
    db.close()

thread_drive = threading.Thread(target=motor_drive)
thread_drive.daemon = True
thread_drive.start()

downward = threading.Thread(target=panel_downward)
downward.start()
